...
blogRoute.post("/", JWTAuthMiddleware, async (req, res, next) => {
  try {
    const { author: authorEmail, ...blogData } = req.body;

    // Check if the author exists
    const existingAuthor = await Author.findOne({ email: authorEmail });
    if (!existingAuthor) {
      return res
        .status(404)
        .send({ message: "Author with the provided email does not exist." });
    }

    // Replace email with the author's ObjectId
    blogData.author = existingAuthor._id;
...
blogRoute.post(
  ...
  async (req, res, next) => {
    try {
      const { author: authorEmail, ...commentsData } = req.body;
      // Check if the author exists
      const existingAuthor = await Author.findOne({ email: authorEmail });
      if (!existingAuthor) {
        return res
      ...
      // Replace email with the author's ObjectId
      commentsData.author = existingAuthor._id;
